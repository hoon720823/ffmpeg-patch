...
#include "libavfilter/zmq_enc_cmd.h"
...
#if CONFIG_ZMQ_FILTER || CONFIG_AZMQ_FILTER
/* fms-patch: ZMQ -> encoder bridge.
 *
 * Receives commands of the form ("@enc_<idx>", "<command>", "<value>")
 * forwarded by libavfilter/f_zmq.c and mutates the matching output stream's
 * AVCodecContext. The libavcodec wrapper's per-encoder reconfig_encoder()
 * picks up the change on the next SendFrame, calling the encoder's native
 * reconfig API (x264_encoder_reconfig / nvEncReconfigureEncoder / ...).
 *
 * <idx> selects an output stream by walking output_files in order:
 * indices 0..output_files[0]->nb_streams-1 belong to the first output
 * file, then continue into output_files[1], etc. This matches ffmpeg's
 * -map ordering for the typical fms-agent use case (single output file
 * with one or more streams).
 *
 * Supported commands: "b" / "bitrate", "maxrate", "minrate", "bufsize".
 * Values accept libav-style suffixes: 1000, 1000k, 2M.
 */
static void fms_apply_enc_cmd(const char *target, const char *command,
                              const char *arg, void *opaque)
{
    int idx = -1;
    ...
    if (sscanf(target, "@enc_%d", &idx) != 1 || idx < 0) {
        av_log(NULL, AV_LOG_WARNING,
               "[fms-patch] invalid enc target '%s'\n", target);
        return;
    }

    scanned = 0;
    for (int fi = 0; fi < nb_output_files; fi++) {
        OutputFile *of = output_files[fi];
        if (!of)
            continue;
        if (idx < scanned + of->nb_streams) {
            ost = of->streams[idx - scanned];
            break;
        }
        scanned += of->nb_streams;
    }
    if (!ost) {
        av_log(NULL, AV_LOG_WARNING,
               "[fms-patch] enc target '%s' out of range (%d total streams)\n",
               target, scanned);
        return;
    }
    if (!ost->enc || !ost->enc->enc_ctx) {
        av_log(NULL, AV_LOG_WARNING,
               "[fms-patch] enc target '%s' has no encoder context (copy mode?)\n",
               target);
        return;
    }
    cc = ost->enc->enc_ctx;
    ...
    if (end == arg) {
        av_log(NULL, AV_LOG_WARNING,
               "[fms-patch] cannot parse value '%s'\n", arg);
        return;
    }
    if      (*end == 'k' || *end == 'K') v *= 1000.0;
    else if (*end == 'M' || *end == 'm') v *= 1000000.0;
    else if (*end == 'G' || *end == 'g') v *= 1000000000.0;
    value = (int64_t) v;

    if (!strcmp(command, "b") || !strcmp(command, "bitrate")) {
        cc->bit_rate = value;
    } else if (!strcmp(command, "maxrate")) {
        cc->rc_max_rate = value;
    } else if (!strcmp(command, "minrate")) {
        cc->rc_min_rate = value;
    } else if (!strcmp(command, "bufsize")) {
        cc->rc_buffer_size = (int) value;
    } else {
        av_log(NULL, AV_LOG_WARNING,
               "[fms-patch] unknown enc command '%s' (target=%s)\n",
               command, target);
        return;
    }

    av_log(NULL, AV_LOG_INFO,
           "[fms-patch] enc applied: idx=%d %s=%" PRId64 "\n",
           idx, command, value);
}
#endif /* CONFIG_ZMQ_FILTER || CONFIG_AZMQ_FILTER */
...
#if CONFIG_ZMQ_FILTER || CONFIG_AZMQ_FILTER
    /* fms-patch: register the encoder-command callback so ZMQ commands
     * targeted at "@enc_*" can mutate the matching encoder's
     * AVCodecContext. See fms_apply_enc_cmd() above. */
    av_zmq_set_enc_command_callback(fms_apply_enc_cmd, NULL);
#endif
